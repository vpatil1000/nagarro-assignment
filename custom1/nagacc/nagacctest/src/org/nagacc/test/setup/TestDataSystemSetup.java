...
import org.nagacc.test.constants.NagaccTestConstants;
import org.nagacc.test.orders.AcceleratorTestOrderData;
 ...
 */
@SystemSetup(extension = NagaccTestConstants.EXTENSIONNAME)
public class TestDataSystemSetup extends AbstractSystemSetup
{
	...
	public void createProjectData(final SystemSetupContext context)
	{
		if (getBooleanSystemSetupParameter(context, CREATE_TEST_DATA))
		{
			...
			importImpexFile(context, "/nagacctest/import/qa-test-users.impex");

			LOG.info("Creating Reviews...");
			importImpexFile(context, "/nagacctest/import/reviews.impex");

			LOG.info("Creating Test Payment Subscriptions...");
			getAcceleratorTestOrderData().createPaymentInfos();

			LOG.info("Creating CS Tickets...");
			importImpexFile(context, "/nagacctest/import/cstickets.impex");
			...
			importImpexFile(context, "/nagacctest/import/csquotes.impex");
		}
	}
}
